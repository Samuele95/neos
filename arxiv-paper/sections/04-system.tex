...
\bigskip\noindent\textbf{Step~1: Create a Session.}
...
\bigskip\noindent\textbf{Step~2: Inject Patterns.}
\nopagebreak[4]\smallskip
...
\bigskip\noindent\textbf{Step~3: Run Dynamics.}
\nopagebreak[4]\smallskip
...
\clearpage
\noindent\textbf{Step~4: Collapse to Insight.}
\nopagebreak[4]\smallskip
...
\bigskip\noindent\textbf{The Same Cycle, Different Domain.}
\nopagebreak[4]\smallskip

\noindent
The four-step cycle is domain-agnostic.  To illustrate, the following
excerpt applies the identical workflow to \emph{code security analysis},
using patterns drawn from a vulnerability assessment.

\begin{terminalbox}[Code Security Analysis --- Inject, Cycle, Collapse]
\begin{lstlisting}[style=neosshell]
...
[CYCLES 1-3]
  1: C=0.65, 4 patterns
  2: C=0.78 [ATTRACTOR-EMERGED] "sql_injection_vulnerability"
     Core: {sql_query, user_input, string_concat, no_validation}
  3: C=0.82 [HIGH] -- attractor stable

nf collapse --strategy attractor
[COLLAPSE] Dominant: sql_injection_vulnerability (C=0.82)
  FINDING: SQL Injection Vulnerability
  All patterns mutually reinforce.  Confidence: HIGH (0.82)
\end{lstlisting}
\end{terminalbox}

\noindent
Four security observations---query construction, user input, string
concatenation, and missing validation---enter the field as patterns.
Within three cycles, resonance amplification clusters them into a single
attractor that the system names \texttt{sql\_injection\_vulnerability}.
Collapse extracts a structured finding with a confidence score.  The
domain changed; the cycle did not.
...
Section~\ref{sec:advanced} extends this foundation with multi-field
orchestration, versioned thinking, and adjustable autonomy---each
illustrated with terminal output from a distinct application domain.
